...
# =========================
# Carga opcional de instaloader (Instagram)
# =========================
try:
    import instaloader
...
def ig_shortcode_from_url(url: str) -> Optional[str]:
    """Extrae el shortcode del URL de Instagram (reel/post)."""
    try:
...
def get_instagram_caption(url: str) -> str:
    """Intenta obtener el caption: (1) instaloader si disponible, (2) meta og:description."""
    # 1) instaloader
    if HAS_INSTALOADER:
        try:
...
def parse_recipe_from_caption(caption: str) -> Dict[str, Any]:
    """Separa título, porciones, tiempo, ingredientes y procedimiento (ES/EN)."""
    ...
    if len(ing_split) >= 3:
        ...
        # Si no viene con secciones, tratar cada línea con viñetas como ingrediente y el resto como procedimiento
        bullets = [l for l in lines if l.strip().startswith(("-", "•", "*"))]
        if bullets:
            rec["ingredientes"] = [clean_bullet(l) for l in bullets]
            rec["procedimiento"] = [l for l in lines if l and l not in bullets]
...
# =========================
PROTEIN_FAMILIES = {
    "pollo/ave": ["pollo", "chicken", "pechuga", "muslo", "ala", "pavo", "turkey", "hen", "gallina"],
    "res": ["res", "carne de res", "beef", "ternera", "lomo de res", "brisket", "ossobuco", "solomo"],
    "cerdo": ["cerdo", "pork", "tocino", "bacon", "lomo de cerdo", "costilla", "jamón"],
    "pescado": ["pescado", "fish", "salmón", "salmon", "tilapia", "bacalao", "merluza", "atún", "tuna", "trucha", "corvina", "dorado", "pargo", "sardina"],
    "mariscos": ["camarón", "camaron", "shrimp", "langostino", "calamar", "pulpo", "mejillón", "mejillon", "almeja", "ostión", "ostion", "cangrejo", "jaiba", "scallop", "vieira"],
    "frijoles/legumbres": ["frijol", "frijoles", "beans", "garbanzo", "chickpea", "lenteja", "lentil", "habas", "poroto", "porotos", "alubia", "judía", "judia"],
    "huevo": ["huevo", "huevos", "egg", "eggs"],
    "soya/tofu": ["soya", "soja", "tofu", "tempeh", "edamame"],
}
...
def generar_plan_mensual(
    ...
    """
    Reglas:
    - No repetir SOPA ni FAMILIA de PROTEÍNA en días consecutivos.
    - Viernes: familia 'pescado' (si activado y hay recetas).
    - Jueves: familia 'frijoles/legumbres' (si activado y hay recetas).
    """
    ...
    # Extender proteínas con familia
    prot_ext = []
    for p in proteinas:
    ...
    for d in range(1, ndays + 1):
        ...
        weekday_idx = day_date.weekday()  # 0=Lun ... 6=Dom
        ...
        # PROTEÍNA (familia requerida y no repetir familia del día anterior)
        prot_pool = prot_ext[:]
        if required_family:
        ...
        # GUARNICIÓN / ARROZ / ENSALADA / POSTRE
        if guarniciones:
...
if pestanas == "Nueva receta":
    ...
    caption_manual = st.text_area(
        ...
        value=caption_detectado,
        height=200
    )

    # Parseo automático del caption si hay
...
# =========================
    ...
    if not recetas:
        ...
        rec_filtradas = recetas if cat_sel == "Todas" else [r for r in recetas if r.get("categoria") == cat_sel]
        ...
        for idx, r in enumerate(rec_filtradas):
            ...
            with st.expander(f"{r.get('categoria','?')} · {r.get('titulo','(sin título)')}"):
                ...
                if st.session_state.get(key_base+"_editing", False):
                    ...
                    ncat = st.selectbox(
                        "Categoría",
                        ["Sopa","Proteína","Arroz","Guarnición","Ensalada","Postre"],
...
# =========================
    ...
    if not recetas:
        ...
        with coly:
            st.download_button(
                label="⬇️ Descargar TODAS en JSON",
                data=json.dumps(recetas, ensure_ascii=False, indent=4),
